...
const emptyForm = { customerId: '', addressId: '', jobTypeId: '', date: '', time: '', duration: '', price: '', status: 'scheduled' as Appointment['status'], paymentStatus: 'unpaid' as Appointment['payment_status'], notes: '' };
...
export default function CalendarPage() {
  ...
  function openEdit(appointment: Appointment) {
    ...
    setForm({ customerId: appointment.customer_id, addressId: appointment.customer_address_id, jobTypeId: appointment.job_type_id, date: local.date, time: local.time, duration: String(appointment.duration_minutes), price: String(appointment.price), status: appointment.status, paymentStatus: appointment.payment_status, notes: appointment.notes ?? '' });
  ...
  async function saveAppointment(e: React.FormEvent) {
    ...
    if (form.date && form.time) {
      try { if (!editing && new Date(israelLocalToIso(form.date, form.time)).getTime() <= Date.now()) { nextErrors.date = 'מועד הפגישה חייב להיות בעתיד.'; nextErrors.time = 'מועד הפגישה חייב להיות בעתיד.'; } }
    ...
    try {
      ...
      if (!editing && new Date(startsAt).getTime() <= Date.now()) throw new Error('Past appointment');
      const values: Record<string, unknown> = { customer_id: form.customerId, customer_address_id: form.addressId, job_type_id: form.jobTypeId, starts_at: startsAt, duration_minutes: Number(form.duration), price: Number(form.price), payment_status: form.paymentStatus, notes: form.notes.trim() || null };
      if (editing) {
        values.status = form.status;
        if (form.status === 'scheduled') { values.started_at = null; values.completed_at = null; }
        if (form.status === 'in_progress') { values.started_at = editing.started_at ?? new Date().toISOString(); values.completed_at = null; }
      }
      const query = editing ? getSupabase().from('appointments').update(values).eq('id', editing.id).eq('status', editing.status) : getSupabase().from('appointments').insert({ ...values, status: 'scheduled' });
  ...
  return <>
    <div className="topline"><div><h1>יומן</h1><div className="sub">יצירה וצפייה בפגישות לפי שעון ישראל</div></div>{!editing && <button className="btn btn-primary" disabled={saving} onClick={openCreate}>+ פגישה חדשה</button>}</div>
    ...
    {show && <form noValidate className="card appointment-form" onSubmit={saveAppointment}><h2 className="section-title">{editing ? 'עריכת פגישה' : 'פגישה חדשה'}</h2><fieldset className="form-grid" disabled={saving}>
      ...
      <label>תאריך<input className="input" aria-invalid={!!fieldErrors.date} type="date" min={editing ? undefined : todayInIsrael} required value={form.date} onChange={e => { setForm({ ...form, date: e.target.value }); setFieldErrors(current => ({ ...current, date: '', time: '' })); setFormError(''); }} /><FieldError message={fieldErrors.date} /></label>
      ...
      {editing && <label>סטטוס פגישה<select className="input" value={form.status} onChange={e => setForm({ ...form, status: e.target.value as Appointment['status'] })}><option value="scheduled">מתוכננת</option>{editing.status !== 'scheduled' && <option value="in_progress">בתהליך</option>}{editing.status === 'completed' && <option value="completed">הושלמה</option>}</select><span className="form-hint">אפשר להחזיר פגישה לסטטוס קודם במקרה של טעות.</span></label>}
    ...
    {!editing && <><div className="calendar-controls card"><div className="view-tabs">{([['agenda','רשימה'],['day','יום'],['week','שבוע']] as [CalendarView,string][]).map(([value,label]) => <button key={value} className={`view-tab ${calendarView === value ? 'active' : ''}`} onClick={() => setCalendarView(value)}>{label}</button>)}</div>{calendarView !== 'agenda' && <div className="date-navigation"><button className="btn" onClick={() => setFocusDate(addDays(focusDate, calendarView === 'week' ? -7 : -1))}>הקודם</button><button className="btn" onClick={() => setFocusDate(todayInIsrael)}>היום</button><strong>{calendarView === 'day' ? dateFormatter.format(new Date(israelLocalToIso(focusDate, '12:00'))) : `שבוע שמתחיל ${weekStart}`}</strong><button className="btn" onClick={() => setFocusDate(addDays(focusDate, calendarView === 'week' ? 7 : 1))}>הבא</button></div>}</div>
