...
function AppointmentCard({ appointment, attachmentUrls, onEdit, onCancel, onStatus, onAddNote, onPayment, onReset }: { appointment: Appointment; attachmentUrls: Record<string, string>; onEdit: (appointment: Appointment) => void; onCancel: (appointment: Appointment) => void; onStatus: (appointment: Appointment) => void; onAddNote: (appointment: Appointment) => void; onPayment: (appointment: Appointment, paymentStatus: Appointment['payment_status']) => void; onReset: (appointment: Appointment) => void }) {
...
export default function CalendarPage() {
  ...
  async function resetAppointment(appointment: Appointment) {
    if (saveLock.current || appointment.status !== 'completed') return;
    saveLock.current = true; setSaving(true); setError(''); setMessage('');
    try {
      const { error } = await getSupabase().from('appointments').update({ status: 'scheduled', started_at: null, completed_at: null }).eq('id', appointment.id).eq('status', 'completed').select('id').single();
      if (error) throw error;
      setMessage('הפגישה הוחזרה למצב מתוכנן.'); await load();
    } catch { setError('החזרת הפגישה למצב מתוכנן נכשלה. רעננו ונסו שוב.'); }
    finally { saveLock.current = false; setSaving(false); }
  }
